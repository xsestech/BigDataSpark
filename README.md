...
Необходимо реализовать ETL-пайплайн с помощью Spark, который трансформирует данные из источника (файлы mock_data.csv с номерами) в модель данных снежинка в PostgreSQL, а затем на основе модели данных снежинка создать ряд отчетов по данным в одной из NoSQL базах данных обязательно и в нескольких других опционально (будет бонусом). Каждый отчет представляет собой отдельную таблицу в NoSQL БД.
...
Все JAR-зависимости подтягиваются командой `make download_jars` (вызывается автоматически из `make up`).

### Сервисы и порты

| Сервис       | Порт(ы)     | Креды                   | Назначение                          |
|--------------|-------------|-------------------------|-------------------------------------|
| PostgreSQL   | 5432        | admin / admin           | raw_sales + snowflake schema        |
| ClickHouse   | 8123, 9000  | admin / admin           | 6 MergeTree-отчётов                 |
| Cassandra    | 9042        | cassandra / cassandra   | 6 CQL-таблиц                        |
| Neo4j        | 7474, 7687  | neo4j / testtest1       | 6 типов нод                         |
| MongoDB      | 27017       | admin / admin           | 6 коллекций                         |
| Valkey       | 6379        | (без auth)              | 6 наборов HASH-ключей               |
| Spark        | —           |                         | apache/spark:3.5.3, local[*]        |

Если какой-то порт занят на хосте — поменяйте маппинг в `docker-compose.yml`.
...
make submit_snowflake
make submit_reports
make submit_cassandra
make submit_neo4j
make submit_mongo
make submit_valkey
make submit_all
